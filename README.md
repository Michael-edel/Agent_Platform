...
Agent Platform для финансовых операций: платформа, которая принимает входящие документы (например, счета в PDF), превращает их в управляемые артефакты и связывает их с дальнейшими действиями в “денежном контуре”.

Первый практический use-case — **контур платежей**: создание платёжного поручения с **PaymentId** как якорем, отправка на согласование (бухгалтер → директор), фиксация аудита и дальнейшие шаги (экспорт/интеграции).

Фокус проекта — **трассируемость, контроль и автоматизация**: прозрачный аудит “document → payment → approval”, предсказуемые статусы, наблюдаемость (`/health`, `/metrics`) и готовность к интеграциям (например, 1С) без потери корреляции по PaymentId.

Также в проекте есть **Payment lifecycle & audit timeline**: для платежа можно получить timeline событий (read-only), чтобы понимать что и когда происходило.

Также есть **Reconciliation (manual first)**: платёж помечается как реально оплаченный только вручную (событие `payment.reconciled`), а импорт банковской выписки и suggestions используются как помощники, но не меняют платежи автоматически.
...
- В тестовом режиме (`CYBERPLAT_TESTING=1` или `PYTEST_CURRENT_TEST`) для SQLite автоматически создаётся schema product layer (включая `artifact_states` и др.).
