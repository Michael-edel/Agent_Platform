...
async def startup_event():
    ...
    # Инициализация product/UI layer repositories
    from cyberplat.product.infrastructure.repositories import (
        ArtifactStateRepositoryImpl,
        ExportRepositoryImpl
    )
    
    product_db_path = os.getenv("PLATFORM_DB_PATH", settings.db_path if settings else "platform.db")
    app.state.artifact_state_repo = ArtifactStateRepositoryImpl(db_path=product_db_path)
    app.state.export_repo = ExportRepositoryImpl(db_path=product_db_path)
    
    logger.info("Product/UI layer repositories инициализированы")
...
# ============================================
# Product/UI Layer: Documents & Invoices API
# ============================================


@app.get("/api/v1/documents", response_model=DocumentsListResponse)
async def get_documents(
    ...
    artifact_state_repo = getattr(request.app.state, "artifact_state_repo", None)
    if not artifact_state_repo:
        raise HTTPException(status_code=500, detail="Product layer not initialized")
...
@app.get("/api/v1/documents/{document_id}", response_model=DocumentDetailDTO)
async def get_document_detail(
    ...
    if not artifact_state_repo or not artifact_service:
        raise HTTPException(status_code=500, detail="Product layer not initialized")
    ...
    result = use_case.execute(tenant_id=x_tenant_id, artifact_id=document_id)
...
@app.post("/api/v1/documents/{document_id}/run-ocr", response_model=AgentRunResponse)
async def run_ocr_on_document(
    ...
    if not artifact_state_repo or not artifact_service or not agent_registry:
        raise HTTPException(status_code=500, detail="Product layer or agents not initialized")
...
@app.get("/api/v1/invoices", response_model=InvoicesListResponse)
...
@app.get("/api/v1/invoices/{invoice_id}", response_model=InvoiceDetailDTO)
async def get_invoice_detail(
    ...
    result = use_case.execute(tenant_id=x_tenant_id, artifact_id=invoice_id)
...
@app.post("/api/v1/invoices/{invoice_id}/confirm", response_model=ConfirmInvoiceResponse)
async def confirm_invoice(
    ...
    success, error_message = use_case.execute(tenant_id=x_tenant_id, artifact_id=invoice_id)
...
@app.post("/api/v1/invoices/{invoice_id}/export", response_model=ExportInvoiceResponse)
async def export_invoice(
    ...
    artifact_state_repo = getattr(request.app.state, "artifact_state_repo", None)
    export_repo = getattr(request.app.state, "export_repo", None)
    
    if not artifact_state_repo or not export_repo:
        raise HTTPException(status_code=500, detail="Product layer not initialized")
