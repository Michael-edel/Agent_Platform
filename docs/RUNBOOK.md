...
### Диагностика approval ошибок

**1. Проверить статус поручения:**
```bash
curl "http://localhost:8000/api/v1/payments/orders/{order_id}" \
  -H "X-Tenant-ID: tenant-123"
```

**2. Проверить шаги согласования:**
- Через payment_service.get_approvals(order_id) (в коде)
- Или через payment_approvals таблицу в БД

**3. Типичные ошибки:**

| Ошибка | Причина | Решение |
|--------|---------|---------|
| "Текущий шаг требует роль 'X'" | Неправильная роль при approve | Использовать правильную роль из required_role |
| "Платёжное поручение уже отправлено" | Повторный submit | Проверить текущий статус |
| "Платёжное поручение должно быть одобрено" | Экспорт неодобренного | Сначала одобрить все шаги |

### Диагностика export ошибок

**1. Проверить статус:**
- `status = "approved"` перед экспортом
- `status = "exported"` после успешного экспорта

**2. CSV экспорт:**
- Файл создаётся в `out/exports/payment_{order_id}_{export_id}.csv`
- Проверить права на запись в директорию

**3. 1С экспорт:**
- Проверить настройки 1С интеграции (enabled, base_url, credentials)
- Проверить логи на ошибки OneCClient
- Проверить метрики: `payment_export_total{format="onec",status="failed"}`

### Метрики

**Prometheus:**
- `payment_orders_total{status}` — количество поручений по статусам
- `payment_approval_latency_seconds` — задержка согласования
- `payment_export_total{format,status}` — результаты экспорта
